...
volatile extern int Hit_Pos = X_BED_SIZE; //rewinder
...
void Endstops::event_handler() {
  ...
  uint16_t Hit_Val = X_BED_SIZE; //rewinder
  ...
  if (hit_state) {
    ...
    if (Hit_Pos == 0) { //rewinder
      Hit_Val = X_BED_SIZE;
      Hit_Val = planner.triggered_position_mm(_AXIS(A));
      if ((Hit_Val != 0) && (Hit_Val != X_BED_SIZE))  { 
        Hit_Pos = Hit_Val;
        //if (Hit_Pos != X_BED_SIZE) {
        SERIAL_ECHOLNPAIR(" Interrupt at :", Hit_Pos); 
        //}
      }
      else {
        if (Hit_Pos == 0) {Hit_Pos = X_BED_SIZE;}
        SERIAL_ECHOLNPAIR(" No interrupt :", Hit_Pos); 
      }
    }
      
      // if (Hit_Pos != X_BED_SIZE) {
      //   SERIAL_ECHOLNPAIR(" Hey, an Interrupt hit at :", Hit_Pos); 
      // }
