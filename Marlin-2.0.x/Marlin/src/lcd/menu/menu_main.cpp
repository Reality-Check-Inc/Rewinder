...
void menu_main() {
  ...
  if (!(planner.movesplanned() || printingIsActive())) { //rewinder
    Hit_Pos = X_BED_SIZE;
  }
  ...
  if (busy) {
    ...
    SUBMENU(MSG_REWINDER, menu_rewinder); //rewinder
    ...
    //SUBMENU(MSG_MOTION, menu_motion); //rewinder
  }
  ...
  //SUBMENU(MSG_CONFIGURATION, menu_configuration); //rewinder

  //#if ENABLED(CUSTOM_USER_MENUS)  //rewinder - moved to the top of the sub menu
